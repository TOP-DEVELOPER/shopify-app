...
import { ImageMajor, AlterMinor } from "@shopify/polaris-icons";
...
import { isProductVariantResourcePayload } from "@shopify/app-bridge/actions/Navigation/Redirect";
...
export function QRCodeForm({ QRCode: InitialQRCode }) {
  ...
  const {
    ...
    fields: {
      title: useField({
        value: QRCode?.title || "",
        validates: [notEmptyString("Please name your QRcode")],
      }),
      productId: useField({
  ...
  */
  const handleProductChange = useCallback(({ selection }) => {
    ...
    variantId.onChange(selection[0].variantId[0].id);
 ...
 */
  ...
  const goToDestination = useCallback(() => {
    ...
    const targetURL = deletedProduct || destination.value[0] === "product" ? productViewURL(data) : productCheckoutURL(data);
...
function productViewURL({
  host,productHandle,discountCode
}){
...
function productCheckoutURL({host,variantId,quantity=1,discountCode}){
  ...
  const  id=variantId.replace(
    /gid:\/\/shopify\/ProductVariant\/([0-9]+)/,"$1"
  );
